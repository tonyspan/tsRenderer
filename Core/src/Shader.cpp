...
const std::vector<ShaderModule*> Shader::GetShaderModules() const
{
	std::vector<ShaderModule*> result(m_ShaderModules.size());

	std::ranges::transform(m_ShaderModules, result.begin(), [](const auto& ptr) { return ptr.get(); });
